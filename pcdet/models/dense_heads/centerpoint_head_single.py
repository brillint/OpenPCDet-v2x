...
class CenterHead(nn.Module):
    ...
    def cuda_to_cpu(self, heatmaps, anno_boxes, inds, masks):
        for idx, ht in enumerate(heatmaps):
            for idy in range(len(ht)):

                # print(heatmaps[idx])
                heatmaps[idx][idy] = heatmaps[idx][idy].cpu()
        for idx, ht in enumerate(anno_boxes):
            for idy in range(len(ht)):
                # print(heatmaps[idx])
                anno_boxes[idx][idy] = anno_boxes[idx][idy].cpu()

        for idx, ht in enumerate(inds):
            for idy in range(len(ht)):
                # print(heatmaps[idx])
                inds[idx][idy] = inds[idx][idy].cpu()

        for idx, ht in enumerate(masks):
            for idy in range(len(ht)):
                # print(heatmaps[idx])
                masks[idx][idy] = masks[idx][idy].cpu()
    ...
    def assign_targets(self, gt_boxes):
        ...
        # heatmaps, anno_boxes, inds, masks = self.cuda_to_cpu(
        ...
        # heatmaps = torch.tensor(heatmaps[0][0],device='cpu')
        # heatmaps = np.matrix(heatmaps)
        # heatmaps = np.array(heatmaps[0][0].cpu())
        heatmaps = np.array(heatmaps).transpose(1,0).tolist()

        # heatmaps = heatmaps.transpose(1, 0).tolist()
